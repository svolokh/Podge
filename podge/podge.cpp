...
static const auto desired_camera_width(5.0f);
static const auto desired_camera_height(8.0f);
...
#define PODGE_REGISTER_COMPONENT(T) static ::podge::registry_init CONCAT(init_, __COUNTER__, _) (::podge::registry_init::component_tag(), [](::podge::registry_init::register_component_fn register_component) { \
    register_component.do_register<T>(registry); \
})
#define PODGE_REGISTER_SYSTEM(T) static ::podge::registry_init CONCAT(init_, __COUNTER__, _) (::podge::registry_init::system_tag(), [](::podge::registry_init::register_system_fn register_system) { \
    register_system(new T()); \
})
#define PODGE_REGISTER_CONTACT_HANDLER(T) static ::podge::registry_init CONCAT(init_, __COUNTER__, _) (::podge::registry_init::contact_handler_tag(), [](::podge::registry_init::register_contact_handler_fn register_contact_handler) { \
...
collision_shape::collision_shape() {
    properties.add_component<systems::fixture_component>();
}
...
object::object(const object &o) {
    for(const auto &p : o.components_) {
        auto res(components_.emplace(p.first, p.second->clone()));
        assert(res.second);
    }
}
...
bool object::has_component(std::type_index cidx) const {
    return components_.find(cidx) != components_.end();
}
...
entity_spec entity_spec::from_tile_xml(pugi::xml_node tile_node, pugi::xml_node tileset_node, const resource_path &cwd) {
    ...
    spec.add_component<systems::core_component>();
    spec.add_component<systems::tile_component>();
    auto &cc(spec.component<systems::core_component>());
...
object &entity::fixture_data(b2Fixture *fixture) {
    ...
    obj->add_component<systems::fixture_component>();
    auto ptr(obj.get());
...
void registry::add_component_to(object &obj, std::type_index cidx) const {
...
const entity_system &registry::system(const char *name) const {
    return *check_iterator(systems_.find(boost::string_view(name)), systems_.end())->second;
}

bool registry::ShouldCollide(b2Fixture *fixtureA, b2Fixture *fixtureB) {
...
void registry::BeginContact(b2Contact *contact) {
...
void registry::EndContact(b2Contact *contact) {
    auto &a(util::entity_from_body(contact->GetFixtureA()->GetBody()));
    auto &b(util::entity_from_body(contact->GetFixtureB()->GetBody()));
    iterate_handlers(a, b, [&](const entity_contact_handler &handler, bool swap) {
        if(swap) {
...
void registry::PreSolve(b2Contact* contact, const b2Manifold* oldManifold) {
    auto &a(util::entity_from_body(contact->GetFixtureA()->GetBody()));
    auto &b(util::entity_from_body(contact->GetFixtureB()->GetBody()));
    iterate_handlers(a, b, [&](const entity_contact_handler &handler, bool swap) {
        entity_contact c(contact, swap);
...
void registry::PostSolve(b2Contact* contact, const b2ContactImpulse* impulse) {
    auto &a(util::entity_from_body(contact->GetFixtureA()->GetBody()));
    auto &b(util::entity_from_body(contact->GetFixtureB()->GetBody()));
    iterate_handlers(a, b, [&](const entity_contact_handler &handler, bool swap) {
        entity_contact c(contact, swap);
...
void registry_init::register_system_fn::operator()(entity_system *s) const {
    auto res(registry.systems_.emplace(s->name(), s).second);
    assert(res);
}

void registry_init::register_contact_handler_fn::operator()(entity_contact_handler *handler) const {
...
void registry_init::register_type_fn::operator()(const char *name, bool has_body, std::initializer_list<const char *> systems) const {
...
void tileset::load_xml(pugi::xml_node tsx_node, const resource_path &cwd) {
    ...
    for(auto tile_xpn : tile_xpns) {
        ...
        auto &tc(spec.component<systems::tile_component>());
        tc.source = this;
...
std::unique_ptr<layer> layer::from_object_layer_xml(pugi::xml_node objectgroup_node, const resource_path &cwd) {
    ...
    for(auto object_xpn : objectgroup_node.select_nodes("./object")) {
        ...
        if(!object_node.attribute("gid").empty()) {
            ...
            const auto &ts_cc(ts_spec.component<systems::core_component>());
        ...
        auto &e_cc(e.component<systems::core_component>());
...
boost::optional<Entity &> layer::entity_by_name_(const std::string &name) {
    for(auto &e : entities_) {
        auto done(false);
        auto &cc(e->component<systems::core_component>());
...
level::level(NVGcontext *vg, int width, int height, float dt) :
    ...
    camera_width_(desired_camera_width),
    camera_height_(desired_camera_height),
    time_(0.0f),
    dt_(dt)
{
}
...
static void run() {
    ...
    check_xml_parse_result(tmx_path, tmx.load_string(get_resource(tmx_path.str()).c_str()));
    ...
    for(;;) {
        ...
        auto update_camera_dims([&lvl, &window]() {
            ...
            if(w < h) {
                // portrait
                cam_width = desired_camera_height*w/h;
                cam_height = desired_camera_height;
            } else {
                // landscape
                cam_width = desired_camera_width;
                cam_height = desired_camera_width*h/w;
            }
